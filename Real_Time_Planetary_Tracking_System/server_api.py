...
# Setup ESP32 connection with better handling
sio = Client()
...
def connect_to_esp32():
    ...
    try:
        if not esp32_connected:
            sio.connect(esp32_ip, wait_timeout=3)
            esp32_connected = True
            print("Connected to ESP32")
    except Exception as e:
        esp32_connected = False
        print(f"Could not connect to ESP32: {e}")
        print("Will retry connection later")

# Try initial connection
try:
    connect_to_esp32()
except Exception:
    pass  # Silently continue if initial connection fails
...
def background_thread():
    ...
    while True:
        ...
        # Send to frontend
        socketio.emit('angles_update', {'altitude': alt, 'azimuth': az})
        
        # Send to ESP32 with error handling
        if esp32_connected:
            try:
                sio.emit('api_update', {'alt': alt, 'az': az})
                print(f"Data sent to ESP32: alt={alt}, az={az}")
            except Exception as e:
        ...
        time.sleep(10)  # Send update every ten seconds
